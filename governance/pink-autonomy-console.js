// Pink Autonomy Console V1 — N0..N5 operational envelope + visible governance console.
// Adds a user-controlled autonomy layer on top of Pink Foundation Approval Engine.
(() => {
  'use strict';
  ...
  function renderEvolution() { const { autonomous, legacy } = getEvolution(); const candidates = autonomous?.candidates || legacy?.openCandidates || []; const experiments = autonomous?.experiments || []; return `<div class="pink-kpi-row"><div class="pink-kpi"><small>Política</small><strong>${escapeHtml(autonomous?.policy || legacy?.policy || 'OBSERVE')}</strong></div><div class="pink-kpi"><small>Candidatos</small><strong>${candidates.length}</strong></div><div class="pink-kpi"><small>Experimentos</small><strong>${experiments.length}</strong></div><div class="pink-kpi"><small>Auto-publicação</small><strong>Bloqueada</strong></div></div><div class="pink-autonomy-note">Fluxo: observar → aprender → propor → isolar → revisar → testar → pedir aprovação → publicar. Produção continua sob autoridade humana.</div><div class="pink-list">${candidates.length ? candidates.slice(0, 12).map(c => `<div class="pink-row"><div><strong>${escapeHtml(c.title || c.kind || 'Melhoria')}</strong><p>${escapeHtml(Array.isArray(c.evidence) ? c.evidence.join(' · ') : c.evidence || `score ${c.score ?? ''}`)}</p></div><span class="pink-chip ${Number(c.score || 0) >= .6 ? 'good' : 'warn'}">${escapeHtml(c.status || c.priority || 'candidate')}</span></div>`).join('') : '<div class="pink-row"><div><strong>Nenhum candidato aberto</strong><p>A Pink continuará observando erros, feedback e desempenho para gerar propostas baseadas em evidência.</p></div><span class="pink-chip good">limpo</span></div>'}</div><div class="pink-autonomy-actions"><button data-pink-export>Copiar pacote de evolução</button><button data-pink-refresh>Atualizar diagnóstico</button></div>`; }
