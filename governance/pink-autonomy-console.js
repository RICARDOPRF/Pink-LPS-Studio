...
(() => {
  ...
  function getEvolution() {
    try {
      const autonomous = window.PinkAutonomousEvolution?.snapshot?.() || null;
      const legacy = window.PinkEvolution?.exportReviewPacket?.() || null;
      const merged = new Map();
      for (const c of [...(legacy?.openCandidates || []), ...(autonomous?.candidates || [])]) {
        const key = c.id || c.key || `${c.kind || 'candidate'}:${c.title || ''}`;
        merged.set(key, { ...(merged.get(key) || {}), ...c });
      }
      return { autonomous: autonomous ? { ...autonomous, candidates: [...merged.values()] } : null, legacy };
  ...
  function refreshEvolutionAfterEvent() { queueMicrotask(() => { if (currentTab === 'evolution' && !document.getElementById('pinkAutonomyOverlay')?.hidden) renderConsole(); }); }
  function install() { if (state.installed) return snapshot(); wrapApprovalEngine(); ensureUi(); state.installed = true; window.addEventListener('pinkfoundation:ready', wrapApprovalEngine); window.addEventListener('pinkplatform:ready', () => { renderBadge(); if (!document.getElementById('pinkAutonomyOverlay')?.hidden) renderConsole(); }); window.addEventListener('pinkevolution:candidate', refreshEvolutionAfterEvent); window.addEventListener('pinkevolution:approved', refreshEvolutionAfterEvent); window.dispatchEvent(new CustomEvent('pinkautonomy:ready', { detail: snapshot() })); return snapshot(); }
