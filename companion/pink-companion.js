...
class CompanionBridge{
 ...
 snapshot(){return {version:'8.0.0',connected:this.connected,session:this.session?{expiresAt:this.session.expiresAt}:null,permissions:this.permissions.snapshot(),capabilities:clone(CAPS),audit:this.audit.slice(-20)}}
}
const bridge=new CompanionBridge();
return {version:'8.0.0',CAPABILITIES:clone(CAPS),PermissionSet,bridge,attach:(transport,opts)=>bridge.attachTransport(transport,opts),invoke:(cap,args,ctx)=>bridge.invoke(cap,args,ctx),disconnect:r=>bridge.disconnect(r),snapshot:()=>bridge.snapshot()};
});
