...
(function (root, factory) {
  ...
  function validateConfig(config) {
    ...
    if (String(cfg.voice?.provider || '') !== 'elevenlabs') warnings.push('voice provider is not elevenlabs');
    if (!/^agent_[a-z0-9]+$/i.test(String(cfg.voice?.agentId || ''))) errors.push('voice.agentId is invalid');
    if (cfg.voice?.branchId && !/^agtbrch_[a-z0-9]+$/i.test(String(cfg.voice.branchId))) errors.push('voice.branchId is invalid');
    if (cfg.voice?.clientCdn && !/^https:\/\/cdn\.jsdelivr\.net\//.test(String(cfg.voice.clientCdn))) warnings.push('voice.clientCdn is outside the pinned jsDelivr policy');
  ...
  class ApprovalEngine {
    constructor(options = {}) {
      this.productionAlwaysRequiresApproval = options.productionAlwaysRequiresApproval !== false;
    }
    assess(action = {}) {
      ...
      return Object.freeze({
        risk,
        approvalRequired,
        reversible: risk === RISK.REVERSIBLE,
        destructive: risk === RISK.DESTRUCTIVE,
        production: risk === RISK.PRODUCTION
      });
    }
  ...
  class RunLedger {
    constructor({ now = () => Date.now(), maxEntries = 500 } = {}) {
      this.now = now;
      this.maxEntries = Math.max(20, Number(maxEntries) || 500);
      this.runs = new Map();
      this.order = [];
      this.sequence = 0;
    }
    start(input = {}) {
      ...
      const run = {
        ...
        risk: normalizeRisk(input.risk || RISK.READ_ONLY),
        status: 'running',
        startedAt: this.now(),
        finishedAt: null,
        error: null,
      ...
      this.order.push(id);
      while (this.order.length > this.maxEntries) this.runs.delete(this.order.shift());
      return this.get(id);
    }
    addEvidence(id, evidence) {
      const run = this.runs.get(String(id));
      if (!run) throw new Error(`Unknown run: ${id}`);
      if (run.status !== 'running') throw new Error(`Cannot mutate terminal run: ${id}`);
      const entry = typeof evidence === 'string' ? { type: 'note', value: evidence } : { ...(evidence || {}) };
      run.evidence.push({ ...entry, at: this.now() });
      return this.get(id);
    }
    finish(id, status, detail = {}) {
      const run = this.runs.get(String(id));
      if (!run) throw new Error(`Unknown run: ${id}`);
      if (run.status !== 'running') throw new Error(`Run already terminal: ${id}`);
      const next = String(status || '');
      if (!TERMINAL.includes(next)) throw new Error(`Invalid terminal status: ${next}`);
      run.status = next;
      run.finishedAt = this.now();
      run.error = detail.error ? String(detail.error) : null;
      if (detail.evidence) run.evidence.push({ type: 'terminal', value: detail.evidence, at: this.now() });
      return this.get(id);
    }
    timeoutStale(maxAgeMs) {
      const age = Math.max(1, Number(maxAgeMs) || 1);
      const now = this.now();
      const timedOut = [];
      for (const id of this.order) {
        const run = this.runs.get(id);
        if (run?.status === 'running' && now - run.startedAt >= age) {
          this.finish(id, 'timeout', { error: 'run exceeded timeout budget' });
          timedOut.push(id);
        }
      }
      return timedOut;
    ...
    assertConsistent() {
      for (const run of this.runs.values()) {
        ...
        if (TERMINAL.includes(run.status) && !Number.isFinite(run.finishedAt)) return false;
        if (run.status !== 'running' && !TERMINAL.includes(run.status)) return false;
      }
      return true;
    }
  }
  ...
  function createFoundation(config) {
    ...
    return Object.freeze({
      ...
      health: () => ({
        ...
        config: validation,
        ledgerConsistent: ledger.assertConsistent()
      })
    });
  }
