...
if (!stage || window.PinkOrbConsole) {
  ...
  const STATE = {
    idle:{core:0xec4899,wire:0x38bdf8,particle:0xec4899,speed:.006,pulse:1.8,scale:1},
    listening:{core:0x06b6d4,wire:0x67e8f9,particle:0x22d3ee,speed:.015,pulse:4,scale:1.1},
    thinking:{core:0x9333ea,wire:0xc084fc,particle:0xa855f7,speed:.035,pulse:6,scale:.97},
    speaking:{core:0xf43f5e,wire:0xfb7185,particle:0xf43f5e,speed:.018,pulse:5,scale:1.08},
    executing:{core:0x10b981,wire:0x34d399,particle:0x10b981,speed:.025,pulse:4.5,scale:1.04},
    error:{core:0xef4444,wire:0xf87171,particle:0xef4444,speed:.008,pulse:5,scale:1},
    success:{core:0x10b981,wire:0x6ee7b7,particle:0x34d399,speed:.01,pulse:2.5,scale:1.04}
  };
  ...
  shell.innerHTML=`
    <div class="pink-orb-statusbar" role="status" aria-live="polite">
      <div class="pink-orb-status-primary">
        <span class="pink-orb-state-dot" aria-hidden="true"></span>
        <div class="pink-orb-status-copy"><small>PINK</small><strong id="pinkOrbStateLabel">PRONTA</strong></div>
      </div>
      <div class="pink-orb-status-secondary">
        <div class="pink-orb-status-copy"><small>LPS STUDIO</small><strong>Central operacional</strong></div>
      </div>
    </div>
  ...
  let disposed=false;
  let visualMode='fallback';
  let disposeVisual=()=>{};

  function initWebGL(){
    let renderer=null,raf=0,ro=null;
    try{
      renderer=new THREE.WebGLRenderer({canvas,antialias:true,alpha:true,powerPreference:'high-performance'});
      renderer.setClearColor(0x000000,0);
      renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,1.7));
      const scene=new THREE.Scene();
      const camera=new THREE.PerspectiveCamera(45,1,.1,100);camera.position.z=7;
      ...
      const count=1050,positions=new Float32Array(count*3),origins=new Float32Array(count*3);
      ...
      function animate(){
        ...
        const t=clock.getElapsedTime(),p=profile(),speaking=currentState()==='speaking';
        const breath=Math.sin(t*p.pulse)*.075,scale=(1+breath)*p.scale*(speaking?1+Math.sin(t*9)*.035:1);
        core.scale.setScalar(scale);wire.scale.setScalar(scale*1.012);inner.scale.setScalar(.94+Math.sin(t*p.pulse*1.2)*.08);
        core.rotation.y+=p.speed;core.rotation.x+=p.speed*.45;wire.rotation.y-=p.speed*.75;rings[0].rotation.z+=p.speed*1.45;rings[1].rotation.z-=p.speed*1.75;rings[2].rotation.y+=p.speed*.9;particles.rotation.y+=p.speed*.28;
        const attr=pGeo.attributes.position,arr=attr.array;
        for(let i=0;i<count;i++){const j=i*3,w=Math.sin(t*1.45+i)*.09;arr[j]=origins[j]*(1+w);arr[j+1]=origins[j+1]*(1+w);arr[j+2]=origins[j+2]*(1+w)}
        attr.needsUpdate=true;
        coreMat.color.lerp(new THREE.Color(p.core),.08);coreMat.emissive.lerp(new THREE.Color(p.core),.08);wireMat.color.lerp(new THREE.Color(p.wire),.08);pMat.color.lerp(new THREE.Color(p.particle),.08);pinkLight.color.lerp(new THREE.Color(p.core),.08);
        renderer.render(scene,camera);
      }
      resize();animate();
      ro=new ResizeObserver(resize);ro.observe(wrap);
      visualMode='webgl';stage.dataset.orbVisual='webgl';shell.classList.add('is-webgl-ready');
      ...
      visualMode='fallback';stage.dataset.orbVisual='fallback';shell.classList.add('is-webgl-fallback');
      wrap.innerHTML='<span class="pink-orb-fallback-orb" aria-hidden="true">✦</span>';
      console.warn('Pink orb WebGL fallback',error);
      try{window.PinkEvolution?.recordIssue?.('orb-webgl',error?.message||String(error))}catch(_){}
      return()=>{};
    }
  }
  ...
  function drawWave(){
    if(disposed||!wctx)return;
    waveRaf=requestAnimationFrame(drawWave);if(document.hidden)return;
    wavePhase+=.075;
    const dpr=Math.min(window.devicePixelRatio||1,2),cssW=wave.clientWidth||760,cssH=wave.clientHeight||72;
    if(wave.width!==Math.round(cssW*dpr)||wave.height!==Math.round(cssH*dpr)){wave.width=Math.round(cssW*dpr);wave.height=Math.round(cssH*dpr)}
    wctx.setTransform(dpr,0,0,dpr,0,0);wctx.clearRect(0,0,cssW,cssH);
    const bars=58,bw=cssW/bars,cy=cssH/2,s=currentState();
    for(let i=0;i<bars;i++){
      let amp=.12;if(s==='speaking')amp=.78+Math.sin(wavePhase*2+i*.3)*.2;else if(s==='listening')amp=.52+Math.sin(wavePhase*1.5+i*.42)*.28;else if(s==='thinking')amp=.32+Math.sin(wavePhase*3+i*.78)*.18;else if(s==='executing')amp=.42+Math.cos(wavePhase*2.4+i*.5)*.2;
      const center=1-Math.abs(i-bars/2)/(bars/2),h=Math.max(4,cssH*amp*center*.82),x=i*bw+bw*.2,y=cy-h/2,g=wctx.createLinearGradient(0,y,0,y+h);
      if(s==='listening'){g.addColorStop(0,'#06b6d4');g.addColorStop(1,'#38bdf8')}else if(s==='thinking'){g.addColorStop(0,'#a855f7');g.addColorStop(1,'#6366f1')}else if(s==='executing'){g.addColorStop(0,'#10b981');g.addColorStop(1,'#34d399')}else{g.addColorStop(0,'#f43f5e');g.addColorStop(.5,'#fb7185');g.addColorStop(1,'#ec4899')}
      wctx.fillStyle=g;wctx.beginPath();if(typeof wctx.roundRect==='function')wctx.roundRect(x,y,bw*.6,h,2);else wctx.rect(x,y,bw*.6,h);wctx.fill();
    }
  }
  drawWave();
  ...
  function destroy(){disposed=true;cancelAnimationFrame(waveRaf);mo.disconnect();disposeVisual();shell.remove();stage.classList.remove('pink-orb-screen');delete stage.dataset.orbVisual}
  window.PinkOrbConsole={version:'1.1.0',destroy,snapshot:()=>({state:currentState(),voiceActive:Boolean(window.PinkSupervisorVoice?.active),ready:!disposed,visualMode})};
