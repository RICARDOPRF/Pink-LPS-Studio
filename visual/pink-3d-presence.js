...
(() => {
  ...
  const stateProfiles = {
    ...
    error:     { speed:.26, pulse:.14,  ring:.36, energy:.54, color:0xff7d9b, accent:0xffb66c }
  };

  const runtime = {
    renderer:null, scene:null, camera:null, field:null, rings:[], nodes:null,
    state:stage.dataset.state || 'idle', running:true, raf:0, start:performance.now(),
  ...
  function setState(state){ if(stateProfiles[state]) runtime.state=state; }
  ...
  function animate(now){
    if(!runtime.running||!runtime.renderer) return;
    const t=(now-runtime.start)/1000, p=profile();
    const motion=reducedMotion?.22:1;
    ...
    runtime.rings.forEach((ring,i)=>{
      ring.rotation.z += .00065 * ring.userData.dir * p.speed * motion;
      ring.rotation.y += .0003 * (i+1) * p.speed * motion;
  ...
  stage.addEventListener('pointerleave',()=>{runtime.pointer.x=0;runtime.pointer.y=0},{passive:true});
  document.addEventListener('visibilitychange',()=>{
    runtime.running=!document.hidden;
    if(runtime.running&&runtime.renderer){runtime.start=performance.now();runtime.raf=requestAnimationFrame(animate)}
    else if(runtime.raf) cancelAnimationFrame(runtime.raf);
  });

  window.Pink3DPresence = {
    setState,
    snapshot:()=>({state:runtime.state,ready:!!runtime.renderer,fallback:runtime.fallback,particles:PARTICLES,lowPower,reducedMotion}),
    destroy:()=>{runtime.running=false;cancelAnimationFrame(runtime.raf);observer.disconnect();ro.disconnect();runtime.renderer?.dispose?.();stage.querySelector('.pink-3d-presence-canvas')?.remove();stage.classList.remove('pink-3d-ready','pink-3d-fallback')}
  };
  boot();
})();
