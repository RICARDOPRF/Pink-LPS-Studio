...
class Gateway{
 constructor(){this.providers=new Map();this.tools=null;this.modelRouter=null;this.runSeq=0;this.registerDefaults()}
 registerDefaults(){[
  {id:'gemini-live',label:'Gemini Live',capabilities:['conversation','voice','vision'],priority:100,status:'implemented'},
  {id:'nvidia-nemotron',label:'NVIDIA Nemotron',capabilities:['reasoning','agents','fallback'],priority:90,status:'implemented'},
  {id:'chatgpt',label:'ChatGPT',capabilities:['orchestration','reasoning','coding'],priority:95,status:'blocked_external'},
 ...
 async invoke(task={}){const capability=this.classify(task);const candidates=this.available(capability);if(!candidates.length){return {status:'blocked_external',reason:`no_connected_provider:${capability}`,capability,providers:this.list().filter(p=>p.capabilities?.includes(capability))}}
  const attempts=[];for(const meta of candidates){const p=this.providers.get(meta.id),started=Date.now();p.lastUsedAt=new Date(started).toISOString();try{const result=await Promise.race([Promise.resolve(p.adapter.invoke(task)),new Promise((_,rej)=>setTimeout(()=>rej(new Error('provider_timeout')),p.timeoutMs||30000))]);p.status='healthy';p.lastLatencyMs=Date.now()-started;p.lastError=null;attempts.push({provider:p.id,status:'completed',latencyMs:p.lastLatencyMs});return {status:'completed',provider:p.id,capability,result,attempts}}catch(e){p.status='degraded';p.lastLatencyMs=Date.now()-started;p.lastError=String(e?.message||e);attempts.push({provider:p.id,status:'failed',error:p.lastError,latencyMs:p.lastLatencyMs})}}
  return {status:'blocked_external',reason:`all_providers_failed:${capability}`,capability,attempts}}
 ...
 snapshot(){return {version:'1.0.0',providers:this.list(),toolRegistry:Boolean(this.tools),modelRouter:Boolean(this.modelRouter)}}
}
const gateway=new Gateway();
function attachBrowser(){if(typeof window==='undefined')return;
 gateway.tools=window.PinkTools||null;gateway.modelRouter=window.PinkMultiAgent||null;
...
if(typeof window!=='undefined'){if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(attachBrowser,0),{once:true});else setTimeout(attachBrowser,0)}
return {version:'1.0.0',gateway,register:m=>gateway.register(m),attach:(id,a,m)=>gateway.attach(id,a,m),mark:(id,s,p)=>gateway.mark(id,s,p),invoke:t=>gateway.invoke(t),tool:(n,c,a,ctx)=>gateway.tool(n,c,a,ctx),snapshot:()=>gateway.snapshot()};
});
