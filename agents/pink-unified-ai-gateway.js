...
class Gateway{
 ...
 snapshot(){return {version:'1.3.0',providers:this.list(),toolRegistry:Boolean(this.tools),modelRouter:Boolean(this.modelRouter)}}
}
...
function attachBrowser(){if(typeof window==='undefined')return;gateway.tools=window.PinkTools||null;gateway.modelRouter=window.PinkMultiAgent||null;
 ...
 if(window.PinkGeminiResearch?.invoke)gateway.attach('gemini-reasoning',{invoke:task=>window.PinkGeminiResearch.invoke(task)},{status:'connected',timeoutMs:60000});
...
if(typeof window!=='undefined'){const attach=()=>setTimeout(attachBrowser,0);if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',attach,{once:true});else attach();window.addEventListener('pinkopenai:ready',attachBrowser);window.addEventListener('pinkplatform:modules-loaded',attachBrowser);window.addEventListener('pinkresearch:ready',attachBrowser)}
return {version:'1.3.0',gateway,register:m=>gateway.register(m),attach:(id,a,m)=>gateway.attach(id,a,m),mark:(id,s,p)=>gateway.mark(id,s,p),invoke:t=>gateway.invoke(t),tool:(n,c,a,ctx)=>gateway.tool(n,c,a,ctx),snapshot:()=>gateway.snapshot()};
});
