...
class Gateway{
 ...
 snapshot(){return {version:'1.1.0',providers:this.list(),toolRegistry:Boolean(this.tools),modelRouter:Boolean(this.modelRouter)}}
}
...
return {version:'1.1.0',gateway,register:m=>gateway.register(m),attach:(id,a,m)=>gateway.attach(id,a,m),mark:(id,s,p)=>gateway.mark(id,s,p),invoke:t=>gateway.invoke(t),tool:(n,c,a,ctx)=>gateway.tool(n,c,a,ctx),snapshot:()=>gateway.snapshot()};
});
