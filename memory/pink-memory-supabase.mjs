...
async function bootstrap(){
  ...
  const enabled=window.PinkPublicConfig?.features?.cloudMemory!==false;
  ...
  window.PinkMemoryCloud=api;
  if(!enabled||!cfg?.url||!cfg?.anonKey){api.status='local-fallback';window.dispatchEvent(new CustomEvent('pinkmemory:ready',{detail:api.health()}));return api}
  try{
    ...
    const {data:memberships,error}=await client.from('pink_tenant_memberships').select('tenant_id,role').eq('user_id',session.user.id).limit(1);
    if(error)throw error;
    const tenantId=memberships?.[0]?.tenant_id;
    if(!tenantId)throw new Error('supabase_memory_tenant_missing');
    const primary=new SupabaseMemoryAdapter({client,tenantId,userId:session.user.id});
    api.service=new core.MemoryService({primary,fallback});api.status='cloud';api.client=client;api.tenantId=tenantId;api.userId=session.user.id;
