...
class SupabaseMemoryAdapter{
  ...
  async upsertRelationship(rel){let subjectId=rel.subject_person_id;if(!subjectId&&rel.subject_name){const person=await this.findPerson(rel.subject_name);subjectId=person?.id}if(!subjectId)throw new Error('relationship_subject_missing');const payload=this.row({subject_person_id:subjectId,target_user_id:rel.target_user_id||null,target_label:rel.target_label||'Paulo',relationship_type:rel.relationship_type,source:rel.source||'self-reported',confidence:rel.confidence??1});const {data,error}=await this.client.from('pink_relationships').upsert(payload,{onConflict:'tenant_id,subject_person_id,target_label'}).select().single();if(error)throw error;return data}
