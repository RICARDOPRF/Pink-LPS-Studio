...
function bindVapiEvents(client){client.on('call-start',()=>{startingCall=false;callActive=true;hideWakeGate();setPinkState('listening');renderVoiceControls();addMessage('assistant','Conexão de voz iniciada. Estou te ouvindo.')});client.on('call-end',()=>{startingCall=false;callActive=false;setPinkState('idle');renderVoiceControls();addMessage('assistant','Chamada encerrada. Quando quiser, pode falar comigo de novo.')});client.on('message',message=>handleVapiMessage(message));client.on('speech-start',()=>{if(callActive)setPinkState('speaking')});client.on('speech-end',()=>{if(callActive)setPinkState('listening')});client.on('error',error=>{console.error('Pink/Vapi error',error);startingCall=false;callActive=false;setPinkState('error');renderVoiceControls();showWakeGate('error');setTimeout(()=>setPinkState('idle'),2400)})}
