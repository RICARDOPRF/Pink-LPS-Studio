...
const source=fs.readFileSync(require('node:path').join(__dirname,'../visual/pink-3d-presence.js'),'utf8').replace("import { createAvatarRuntime } from './avatar/runtime.mjs';", "const createAvatarRuntime=()=>({start(){},update(){},destroy(){}});").replace('await import(THREE_URL)','await Promise.resolve(THREE_MOCK)');
