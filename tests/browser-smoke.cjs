...
async function smoke(browser,name,contextOptions){
 ...
 await page.waitForFunction(()=>Boolean(window.PinkSupervisorVoice&&window.PinkNeuralTTS&&window.PinkOpenAI&&window.PinkAIGateway&&window.PinkIntentRouter&&window.PinkAgentHarness),null,{timeout:12000});
 ...
 const supervisor=await page.evaluate(()=>({exists:Boolean(window.PinkSupervisorVoice),voiceProvider:window.PinkSupervisorVoice?.voiceProvider||null,tts:window.PinkNeuralTTS?.snapshot?.()||null,voiceConfig:window.PinkPublicConfig?.voice||null,openai:window.PinkOpenAI?.getStatus?.()||null,gateway:window.PinkAIGateway?.snapshot?.()||null,intent:Boolean(window.PinkIntentRouter)}));
 assert.strictEqual(supervisor.exists,true,`${name}: supervisor voice missing`);assert.strictEqual(supervisor.voiceProvider,'gemini-tts',`${name}: neural TTS is not supervisor voice provider`);assert.strictEqual(supervisor.tts?.provider,'gemini-tts',`${name}: Gemini TTS client missing`);assert.strictEqual(supervisor.tts?.voice,'Aoede',`${name}: Aoede voice not configured`);assert.strictEqual(supervisor.tts?.endpoint,'pink-tts',`${name}: Pink TTS endpoint mismatch`);assert.strictEqual(supervisor.voiceConfig?.provider,'gemini-tts',`${name}: public voice provider mismatch`);assert.ok(supervisor.gateway?.providers?.some(p=>p.id==='chatgpt'),`${name}: ChatGPT provider missing from gateway`);assert.ok(supervisor.gateway?.providers?.some(p=>p.id==='gemini-reasoning'&&['connected','healthy','degraded'].includes(p.status)),`${name}: grounded web research provider not connected`);assert.strictEqual(supervisor.intent,true,`${name}: intent router missing`);
