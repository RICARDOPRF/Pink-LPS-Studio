...
async function smoke(browser,name,contextOptions){
 ...
 if(name==='desktop'){await page.reload({waitUntil:'domcontentloaded'});await page.waitForFunction(()=>Boolean(window.PinkMemoryCloud&&window.PinkSupervisorVoice&&window.PinkCommandCenter),null,{timeout:12000});recalled=await page.evaluate(()=>window.PinkMemoryCloud.recall('memória persistente',{minScore:.1}));assert.ok(recalled.length>=1,`${name}: cross-page memory persistence failed`);assert.ok(await page.locator('#pinkCommandDock').isVisible(),`${name}: command dock lost after reload`)}
 ...
 console.log(`Browser smoke ${name}: OK (${performance.tier}, core ${operating.version}, memory ${memoryHealth.version}, supervisor active, command center ${commandSnapshot.version})`);await context.close();
}
