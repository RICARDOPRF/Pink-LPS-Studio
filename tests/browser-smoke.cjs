...
async function smoke(browser,name,contextOptions){
 ...
 assert.strictEqual(supervisor.exists,true,`${name}: supervisor voice missing`);assert.strictEqual(supervisor.voiceProvider,'nvidia-magpie',`${name}: NVIDIA Magpie is not supervisor voice provider`);assert.strictEqual(supervisor.tts?.provider,'nvidia-magpie',`${name}: NVIDIA Magpie TTS client missing`);assert.ok(['not-created','running','suspended'].includes(supervisor.tts?.audioContextState),`${name}: neural audio context state missing`);assert.ok(/PT-BR\./.test(supervisor.tts?.voice||''),`${name}: NVIDIA pt-BR voice not configured`);assert.strictEqual(supervisor.tts?.endpoint,'pink-nvidia-tts',`${name}: NVIDIA TTS endpoint mismatch`);assert.strictEqual(supervisor.voiceConfig?.provider,'nvidia-magpie',`${name}: public voice provider mismatch`);assert.ok(supervisor.voiceCount>=3,`${name}: NVIDIA pt-BR voice catalog incomplete`);assert.ok(supervisor.gateway?.providers?.some(p=>p.id==='chatgpt'),`${name}: ChatGPT provider missing from gateway`);assert.ok(supervisor.gateway?.providers?.some(p=>p.id==='gemini-reasoning'&&['connected','healthy','degraded'].includes(p.status)),`${name}: grounded web research provider not connected`);assert.strictEqual(supervisor.intent,true,`${name}: intent router missing`);
