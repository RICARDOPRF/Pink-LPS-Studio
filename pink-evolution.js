(() => {
  ...
  function candidate({kind='improvement',title,evidence='',priority='medium'}={}){
    ...
    if(existing){existing.hits+=1;existing.lastSeenAt=now();existing.evidence=clean(evidence,220)||existing.evidence;save();emit('pinkevolution:candidate',existing);return existing}
    ...
    state.candidates.push(item);save();emit('pinkevolution:candidate',item);return item;
  }
